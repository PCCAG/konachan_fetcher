...
async def main(
    pids: list | tuple,
    sem_times: int = 15,
    imgpath: str = os.getenv("IMG_PATH"),
    ...
    pid_tags_img_path_img_link = await download_imgs(pid_img_link_tags)
...
# 保存数据到本地与数据库
@logger.catch()
def save_img_and_todb(pids, sem_times: int, engine, kimg_table, tags_table):
    global df
    number__ = 0  # 内部变量
    ...
    try:
        if len(pids) == 0:
            ...
            return False
        pid_tags_img_path_img_link = asyncio.run(main(pids=pids, sem_times=sem_times))
...
def mode_a():  # 随机范围下载模式 从范围内生成一堆数量的图片任务
    ...
    try:
        ...
        down_times = os.getenv("times")
        down_times = int(down_times)
        ...
        assert isinstance(down_times, int), "下载次数请输入整数"
    except Exception as e:
        raise e
    ...
    logger.success(f"下载次数:{down_times}")
    ...
    for _ in range(down_times):
        ...
        with engine:
            ...
            save_img_and_todb(pids, sem_times, engine, kimg_table, tags_table)
