...
def save_img_and_todb(pids, engine, kimg_table, tags_table):
    ...
    try:
        if len(pids) == 0:
            ...
            return False
        pid_tags_img_path_img_link = main(pids=pids)
    ...
    for pid, tags, img_path, img_link in pid_tags_img_path_img_link:
        if (
            ...
            for tag in tags.split(" "):
                if len(tag.strip()) != 0:
                    set_tags.add((tag.strip(), pid))
                    # print(tag.strip())

    try:
        set_tags_from_db = set(
            map(
                tuple,
                pandas.read_sql_table(
                    table_name=tags_table, con=engine, columns=["tag", "pid"]
                ).to_records(),
            )
        )

    except ValueError:
        logger.warning(f"{tags_table} 不存在,可能为新表")

        set_tags_from_db = set()

    need_add_set_tags = set_tags - set_tags_from_db

    if set_tags.__len__() - need_add_set_tags.__len__() > 0:
        logger.warning(f"排除了{set_tags.__len__()- need_add_set_tags.__len__()}个重复tag")

    df_tags = pandas.DataFrame(list(need_add_set_tags), columns=["tag", "pid"])

    # df_tags_list_from_db = pandas.read_sql_table(
    #     table_name=tags_table,
    #     con=engine,
    # )["tags"].to_list()

    # df_tags_set_from_db = set(df_tags_list_from_db)
    ...
    for _ in range(times):
        ...
        with engine:
            ...
            try:
                ...
                # )
                df_pid = pandas.read_sql_table(table_name=kimg_table, con=engine)
                df_pid = df_pid["pid"].unique()
            except Exception as e:
                df_pid = []
