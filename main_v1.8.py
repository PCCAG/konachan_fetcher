...
if __name__ == "__main__":
    ...
    def main(pids,sem_times:int=15,wait_time:int=3):
        ...
        async def process_urls(pids):
            ...
            async with sem:
                ...
                #参数
                results=[(i,ii) for i,ii in r if ii != "寄"] #pid_source=[(pid,source),......]
                return results
    ...
    def save_img_and_todb(pids,wait_time:int,sem_times:int,engine,kimg_table,tags_table):
        ...
        sleep(3)"""
        with contextlib.suppress(AttributeError):#AttributeError: 'Connection' object has no attribute 'commit'
            engine.commit()
